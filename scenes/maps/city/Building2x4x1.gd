...
@export var floors: int = 4   # how many Y layers to block (0 = ground only => set to 1)
@export var size_z: int = 1   # tiles along Z (north–south) height
...
func _block_footprint():
	...
	var origin_x := int(floor(global_transform.origin.x / grid.tile_size))
